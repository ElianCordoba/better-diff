...
export function getChanges(codeA: string, codeB: string): Change[] {
  ...
  function loop(minLCS: number) {
    while (true) {
      ...
      const change = matchSubsequence(iterA, iterB, indexA, indexB, indexOfBestResult, lcs);
  ...
  mainLoop: while (true) {
    ...
    if (currentMinLCS <= 1) {
      oneSidedIteration(iterA, ChangeType.deletion)
      oneSidedIteration(iterB, ChangeType.addition)
      break mainLoop;
    }
  }
...
function matchSubsequence(iterA: Iterator, iterB: Iterator, indexA: number, indexB: number, indexOfBestResult: number, lcs: number): Change | undefined {
  ...
  while (index < indexOfBestResult + lcs) {
    ...
    const safeToIgnore = a.lineNumberStart === b.lineNumberStart || a.lineNumberEnd === b.lineNumberEnd
    if (safeToIgnore) {
      continue;
    }
